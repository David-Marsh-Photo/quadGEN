...
2. Start the watcher in a long‑running terminal:
   ...
   ```
   The script prints `watch-runner is running` once it is ready. Leave this process running; it listens for triggers written to `runner/trigger.json` and executes `npm run test:e2e:gate` (which runs `node run-playwright-test.js`).
