...
- **Reference comparison**: Loading a reference `.quad` first applies its file type/size guards, then uses the same exact structural parser as editable imports and matches the parsed channels against the active printer. At least one channel must match before the dashed comparison curve is populated.
